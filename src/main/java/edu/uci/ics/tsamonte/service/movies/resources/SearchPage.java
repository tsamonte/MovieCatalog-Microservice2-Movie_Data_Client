...
import edu.uci.ics.tsamonte.service.movies.models.*;
...
//            String servicePath = "http://localhost:5813/api/idm";
            ...
            // note: privilegeResponse.getResultCode()
            // resultCode = 140 means User has sufficient privilege level
            // resultCode = 141 means User has insufficient privilege level

            // if 141, hidden should always be null; else, keep the original value of hidden
//            Boolean privilegeCheck = hidden;
            ...
            MovieModel[] movies = MovieRecords.retrieveSearch(queryModel);

            Response.ResponseBuilder builder;
            // resultCode = 210; 200 OK; "Found movie(s) with search parameters."
            if (movies != null) {
                responseModel = new MovieResultsResponseModel(Result.FOUND_MOVIES, movies);
            }
            // resultCode = 211; 200 OK; "No movies found with search parameters."
            else {
                responseModel = new MovieResultsResponseModel(Result.NO_MOVIES_FOUND, movies);
            }
